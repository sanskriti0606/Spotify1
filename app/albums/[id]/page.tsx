...
import React from 'react';
import { useGetAlbumDetailsQuery } from '../../../redux/services/rapidapiCore';
import Loader from '../../../components/Preloader';
import TrackList from '../../../components/TrackList';
...
export default function AlbumPage() {
  ...
  const { data, isFetching, error } = useGetAlbumDetailsQuery(id as string);
  const album = data;
  ...
  return (
    <div className="bg-black p-6">
      <h1 className="text-white text-3xl mb-6">{album.name}</h1>
      <ul>
        {album.tracks.items.map((track: any) => (
          <TrackList key={track.id} item={track} />
        ))}
      </ul>
    </div>
  );
